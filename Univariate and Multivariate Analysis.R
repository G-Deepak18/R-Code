...
# Plot cumulative variance vs. eigenvalues
library(ggplot2)

ggplot(variance_df, aes(x = Component)) +
  # Plot only the cumulative variance
...
#Multi regression
# Fitting a linear model predicting Alcohol using Proline and Malic_Acid
model_wine <- lm(Alcohol ~ Proline + Malic, data = wine)
summary(model_wine)
...
# Standardize the data (numeric features only)
diamonds_data <- diamonds[, c("carat", "depth", "table", "x", "y", "z")]
